...
.PHONY: all clean remove install update build format anvil coverage test testLocal testFork testEth deployEth deployArb bridgeEthToArb bridgeArbToEth configPoolEth configPoolArb
...
install :; forge install foundry-rs/forge-std@v1.9.7 && forge install openzeppelin/openzeppelin-contracts@v5.3.0 && forge install smartcontractkit/chainlink-brownie-contracts@1.3.0
...
coverage :; forge coverage --report summary --via-ir
...
# Configure the pool on Ethereum Sepolia
# TODO: Check rate limiter configuration
configPoolEth:
	@echo "Usage: make configPoolEth ETH_SEPOLIA_POOL_ADDRESS=... ARB_SEPOLIA_POOL_ADDRESS=... ARB_SEPOLIA_REBASE_TOKEN_ADDRESS=..."
...
# TODO: Check rate limiter configuration
configPoolArb:
	@echo "Usage: make configPoolArb ARB_SEPOLIA_POOL_ADDRESS=... ETH_SEPOLIA_POOL_ADDRESS=... ETH_SEPOLIA_REBASE_TOKEN_ADDRESS=..."
	@forge script script/ConfigurePool.s.sol:ConfigurePoolScript \
		--sig "run(address,uint64,address,address,bool,uint128,uint128,bool,uint128,uint128)" \
...
bridgeArbToEth:
	...
	@echo "Usage: make bridgeSepolia RECEIVER=0x... TOKEN=0x... AMOUNT=X"
	@forge script script/BridgeTokens.s.sol:BridgeTokensScript \
		--sig "run(address,uint64,address,uint256,address,address)" \
